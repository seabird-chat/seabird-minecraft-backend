...
import chat.seabird.seabird_minecraft_backend.events.AdvancementEvent;
import chat.seabird.seabird_minecraft_backend.events.CommandEvent;
import chat.seabird.seabird_minecraft_backend.events.EmoteEvent;
import chat.seabird.seabird_minecraft_backend.events.MessageReceivedEvent;
import chat.seabird.seabird_minecraft_backend.events.PlayerDiedEvent;
import chat.seabird.seabird_minecraft_backend.events.PlayerJoinedEvent;
import chat.seabird.seabird_minecraft_backend.events.PlayerLeftEvent;
import chat.seabird.seabird_minecraft_backend.events.SystemMessageEvent;
...
public class SeabirdMod {
    ...
    private static Config readConfig() {
        try {
            ...
            if (config.seabirdHost == null) {
                config.seabirdHost = "api.seabird.chat";
            }
    ...
    static void onAdvancement(AdvancementEvent advancementEvent) {
        SeabirdChatIngest.ChatEvent event = SeabirdChatIngest.ChatEvent.newBuilder()
                .setMessage(Common.MessageEvent.newBuilder()
                        ...
                        .setText(String.format(
                                ...
                                advancementEvent.getPlayerName(),
                                advancementEvent.getAdvancementName()
                        ))).build();

        SeabirdMod.outgoingQueue.push(event);
    }

    static void onPlayerDied(PlayerDiedEvent playerDiedEvent) {
        SeabirdChatIngest.ChatEvent event = SeabirdChatIngest.ChatEvent.newBuilder()
                .setMessage(Common.MessageEvent.newBuilder()
                        .setSource(Common.ChannelSource.newBuilder()
                                .setChannelId(SeabirdMod.config.backendChannel)
                                .setUser(Common.User.newBuilder()
                                        .setId("SYSTEM")
                                        .setDisplayName(SeabirdMod.config.systemDisplayName)))
                        .setText(playerDiedEvent.getDeathMessage())).build();

        SeabirdMod.outgoingQueue.push(event);
    }

    static void onPlayerJoined(PlayerJoinedEvent playerJoinedEvent) {
        SeabirdChatIngest.ChatEvent event = SeabirdChatIngest.ChatEvent.newBuilder()
                .setMessage(Common.MessageEvent.newBuilder()
                        .setSource(Common.ChannelSource.newBuilder()
                                .setChannelId(SeabirdMod.config.backendChannel)
                                .setUser(Common.User.newBuilder()
                                        .setId("SYSTEM")
                                        .setDisplayName(SeabirdMod.config.systemDisplayName)))
                        .setText(String.format("%s joined the server.", playerJoinedEvent.getPlayerName()))).build();

        SeabirdMod.outgoingQueue.push(event);
    }

    static void onPlayerLeft(PlayerLeftEvent playerLeftEvent) {
        SeabirdChatIngest.ChatEvent event = SeabirdChatIngest.ChatEvent.newBuilder()
                .setMessage(Common.MessageEvent.newBuilder()
                        .setSource(Common.ChannelSource.newBuilder()
                                .setChannelId(SeabirdMod.config.backendChannel)
                                .setUser(Common.User.newBuilder()
                                        .setId("SYSTEM")
                                        .setDisplayName(SeabirdMod.config.systemDisplayName)))
                        .setText(String.format("%s left the server.", playerLeftEvent.getPlayerName()))).build();

        SeabirdMod.outgoingQueue.push(event);
    }

    public static void onMessage(MessageReceivedEvent messageReceivedEvent) {
        SeabirdChatIngest.ChatEvent event = SeabirdChatIngest.ChatEvent.newBuilder()
                .setMessage(Common.MessageEvent.newBuilder()
                        .setSource(Common.ChannelSource.newBuilder()
                                .setChannelId(SeabirdMod.config.backendChannel)
                                .setUser(Common.User.newBuilder()
                                        .setId(messageReceivedEvent.getPlayerId())
                                        .setDisplayName(messageReceivedEvent.getPlayerName())))
                        .setText(messageReceivedEvent.getMessageText())).build();

        SeabirdMod.outgoingQueue.push(event);
    }

    static void onCommand(CommandEvent commandEvent) {
        switch (commandEvent.getCommandName()) {
            case "me" -> {
                if (commandEvent.getPlayerId() == null || commandEvent.getPlayerName() == null) {
                    return;
                }
                String message = String.join(" ", commandEvent.getArguments());
                EmoteEvent event = new EmoteEvent(commandEvent.getPlayerId(), commandEvent.getPlayerName(), message);
                onEmote(event);
            }
            case "say" -> {
                String text = String.join(" ", commandEvent.getArguments());
                SystemMessageEvent event = new SystemMessageEvent(commandEvent.getSenderDisplayName(), text);
                onSystemMessage(event);
            }
        }
    }

    public static void onEmote(EmoteEvent emoteEvent) {
        SeabirdChatIngest.ChatEvent event = SeabirdChatIngest.ChatEvent.newBuilder()
                .setAction(Common.ActionEvent.newBuilder()
                        .setSource(Common.ChannelSource.newBuilder()
                                .setChannelId(SeabirdMod.config.backendChannel)
                                .setUser(Common.User.newBuilder()
                                        .setId(emoteEvent.getPlayerId())
                                        .setDisplayName(emoteEvent.getPlayerName())))
                        .setText(emoteEvent.getMessage())).build();

        SeabirdMod.outgoingQueue.push(event);
    }

    public static void onSystemMessage(SystemMessageEvent systemMessageEvent) {
        SeabirdChatIngest.ChatEvent event = SeabirdChatIngest.ChatEvent.newBuilder()
                .setMessage(Common.MessageEvent.newBuilder()
                        .setSource(Common.ChannelSource.newBuilder()
                                .setChannelId(SeabirdMod.config.backendChannel)
                                .setUser(Common.User.newBuilder()
                                        .setId("SYSTEM")
                                        .setDisplayName(systemMessageEvent.getSender())))
                        .setText(systemMessageEvent.getText())).build();
